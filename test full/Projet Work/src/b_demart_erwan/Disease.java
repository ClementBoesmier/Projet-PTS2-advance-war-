...
public class Disease {
    ...
    public void draw(){
        for(int x = 0; x < itsPopulation.length-1; x ++)
        {
            for (int y = 0; y < itsPopulation.length-1; y++) {
                itsPopulation[x][y].draw();
            }
        }
    }
    public void setAvatar(int aRow, int aColumn, Rectangle anAvatar){
        itsPopulation[aRow][aColumn].setAvatar(anAvatar);
        if(aRow == 25 && aColumn == 25)
        {
            itsPopulation[aRow][aColumn].updateState(PersonState.INFECTED,0);
        }
    }
}
